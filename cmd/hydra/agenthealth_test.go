...
func healthResponse(t *testing.T, method string, lives []*liveEngine) (int, map[string]interface{}) {
	t.Helper()
	w := httptest.NewRecorder()
	agentHealthHandler(lives)(w, httptest.NewRequest(method, "/health", nil))
...
func TestAgentHealthOKWithNoEngines(t *testing.T) {
	code, body := healthResponse(t, "GET", nil)
	if code != 200 {
...
func TestAgentHealthFailsOnADeadEngine(t *testing.T) {
	code, body := healthResponse(t, "GET", []*liveEngine{{id: "hoard-0", role: "hoard"}})
	if code != 503 {
...
func TestAgentHealthRejectsWrites(t *testing.T) {
	if code, _ := healthResponse(t, "POST", nil); code != 405 {
