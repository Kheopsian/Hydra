        ...
        let reader = async {
            ...
            loop {
                let (size, addr) = match socket.recv_from(&mut buf).await {
                    Ok(v) => v,
                    // Hydra/Typhon patch (Windows): WSAECONNRESET (10054) following an ICMP
                    // port-unreachable is not fatal here — skip it and keep reading.
                    Err(e) if e.kind() == std::io::ErrorKind::ConnectionReset => continue,
                    Err(e) => return Err(e).context("error reading from UDP socket"),
                };
...
impl DhtState {
    ...
    pub fn with_config(mut config: DhtConfig) -> BoxFuture<'static, anyhow::Result<Arc<Self>>> {
        async move {
            ...
            // Hydra/Typhon patch (Windows): suppress WSAECONNRESET (os error 10054) on this
            // UDP socket. An ICMP port-unreachable would otherwise fail the next recv and kill
            // the DHT reader. SIO_UDP_CONNRESET = FALSE restores Unix-like behaviour.
            #[cfg(windows)]
            {
                ...
                let enable: u32 = 0;
                let mut br: u32 = 0;
                // SAFETY: `socket` is a valid, open UDP socket.
                unsafe {
                    WSAIoctl(
